...
## Getting social contacts and demography data
...
Since age is the major driver of susceptibility, we can retain the demography-susceptibility matrix used in the [basic usage vignette](using_finalsize.html). This essentially assumes that all individuals have the same susceptibility as their age group: all infants 0 -- 5 have a susceptibility of 0.9, all children 5 -- 18 have a susceptibility of 0.3, and so on.
...
## Susceptibility variation due to vaccination
...
We can then predict the final size of an epidemic caused by diseases with different R<sub>0</sub>, from the less-transmissible seasonal influenza (R<sub>0</sub> ≈ 1.3) to the highly transmissible Omicron variant of Covid-19 (R<sub>0</sub> ≈ 9.0).
...
```{r echo=FALSE, fig.cap = "Final size of an epidemic outbreak in a population with heterogeneous susceptibility to disease. In this example, susceptibility varies between age groups, as well as within age groups due to vaccination. This example shows that immunisation can strongly limit the spread of epidemic diseases with a low R<sub>0</sub>. This reduction is not only among those who are actually vaccinated, but also among those who are unvaccinated."}
# plot as heatmap
