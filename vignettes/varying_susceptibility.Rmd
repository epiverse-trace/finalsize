...
Populations are often heterogeneous in their susceptibility to a disease. This heterogeneity may be age dependent and vary between age groups. It may also vary within age groups due to risk factors or due to immunisation. Combinations of within- and between-group variation in susceptibility are also likely, and can be incorporated into final size calculations [ [@miller2012] ](http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3506030/).
...
There is substantial **heterogeneity in susceptibility to disease** in a population. You want to know how this heterogeneity could affect the final size of the epidemic.
:::

::: {.alert .alert-secondary}
### What you have {-}

  1. In addition to the disease $R_0$, demography data, and social contact data;
  2. Data on within- and between-group variation in susceptibility to the disease; and
...
This example considers a scenario in which susceptibility to the disease varies between age groups, but not within groups.
A common observation with many diseases is that susceptibility increases with age.
...
```{r echo=FALSE, fig.cap="Final sizes of epidemics in populations wherein susceptibility to the disease is either uniform (green), or heterogeneous (purple), with older individuals more susceptible to the disease.", fig.width=5, fig.height=4}
...
This example considers a scenario in which susceptibility to the disease varies within and between age groups.
Immunisation to a disease through an intervention campaign is a common cause of within-age group variation in susceptibility, and this example can be thought of as examining the effect of vaccination.
...
You also need to model the proportion of each age group that has been immunised, and which therefore has lower susceptibility to the disease. To do this, you can modify the demography-susceptibility distribution matrix.
...
```{r echo=FALSE, fig.cap="Final size of an SIR epidemic with $R_0$ = 2.0, in a population wherein 50% of each age group is immunised against the disease. The immunisation is assumed to reduce the initial susceptibility of each age group by 25%. This leads to both within- and between-group heterogeneity in susceptibility. Vaccinating even 50% of each age group can substantially reduce the epidemic final size in comparison with a scenario in which there is no immunisation (grey). Note that the final sizes in this figure are all below 0.5.", fig.width=5, fig.height=4}
