...
population <- contactdata::age_df_countries(country) |>
  dplyr::filter(age %in% rownames(contacts))
...
vaccinate <- function(priority_order, population, doses) {
  ...
  while (doses > 0) {
    ...
    if (pop_this_group > doses) {
      res[priority_order[i], "vaccinated"] <- res[priority_order[i], "vaccinated"] + doses
      res[priority_order[i], "unvaccinated"] <- res[priority_order[i], "unvaccinated"] - doses
      doses <- 0
    } else {
      res[priority_order[i], "vaccinated"] <- res[priority_order[i], "vaccinated"] + pop_this_group
      res[priority_order[i], "unvaccinated"] <- res[priority_order[i], "unvaccinated"] - pop_this_group
...
scenarios |>
  ...
  ggplot(aes(x = scenario, y = infected, fill = scenario)) +
    geom_col(position = "dodge") +
    theme_minimal() +
    labs(
      x = "Age group",
