...
### What you have {-}
  ...
  1. An estimate of the disease's basic reproductive number $R_0$;
  ...
  4. An estimate of the susceptibility of each demographic group to the disease.

### What you assume {-}

  1. That the disease dynamics can be captured using a Susceptible-Infectious-Recovered (SIR) model.
:::
...
Instead, this example considers a disease with an $R_0$ of 2.0, similar to some strains of seasonal influenza.
...
High uniform susceptibility can be modelled as a matrix with values of 1.0, with as many rows as there are demography groups. The matrix has a single column, representing the single susceptibility group to which all individuals belong.
...
In this example, all individuals are equally susceptible to the disease, and thus the susceptibility matrix has only a single column with identical values.
