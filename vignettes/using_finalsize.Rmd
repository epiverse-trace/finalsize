...
The final size of an epidemic can be calculated using the _finalsize_ package, using methods detailed in [ @andreasen2011 ](https://link.springer.com/article/10.1007/s11538-010-9623-3), [ @miller2012 ](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3506030/), [ @kucharski2014 ](https://journals.plos.org/plospathogens/article?id=10.1371/journal.ppat.1004206), and [@bidari2016](https://www.sciencedirect.com/science/article/pii/S0025556416302802).
...
In the early stages of an epidemic outbreak, a safe assumption might be that all age groups have a similar susceptibility. 
...
Final size calculations also need to know the proportion of each demographic group $\{i\}$ that falls into the susceptibility group $\{j\}$.

Since all individuals in each age group have the same susceptibility, there is no variation within age groups. This can be represented as a single-column matrix, with as many rows as age groups.
...
In this example, all individuals are equally susceptible to infection, and thus the susceptibility matrix has only a single column with identical values.

Consequently, the demography-in-susceptibility matrix has the same dimensions, and all of its values are 1.0.
