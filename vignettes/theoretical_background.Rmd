...
$$
This equation doesn't have an analytical solution, but can be solved numerically with Newton’s method to find the final epidemic size, i.e. the unique solution in (0,1]:
...
We can extend the above formulation to account for variable susceptibility among individuals, following the methods outlined in [Miller (2012)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3506030/). First, we define $p(x)$ to be the probability density for a randomly chosen individual to be of suceptibility type $x$. If we define the probability an individual of type $x$ to be ultimately infected as $\phi(x)$, then the overall proportion of the population to be infected is defined as follows, depending on whether $p(x)$ is a continuous or discrete distribution:
...
We therefore obtain the following relationship between $\phi$ and $\hat\phi$, under the assumption that $S(x,0)$ is near 1 initially:

$$
1-\phi(x) = S(x,0) e^{-R_0 \hat\phi } \\
\hat\phi = 1- \sum_x p(x) e^{-R_0 \hat\phi }  
$$
...
$$
where $\mathbf{R}$ is a matrix with entries $R_{i,j}$. Once again, we can use Newton's method, this time with $\underline\phi$ as a vector, and hence we need to a set of linear equations in step 2.
