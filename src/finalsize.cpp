...
Eigen::ArrayXd solve_final_size_newton(
...
//' 
// [[Rcpp::export]]
Eigen::MatrixXd solve_final_size_internal(const Eigen::MatrixXd &contact_matrix,
                                          const Eigen::VectorXd &demography,
                                          const Eigen::MatrixXd &p_susceptibility,
                                          const Eigen::MatrixXd &susceptibility,
                                          const bool adapt_step = true,
  ...
  // Eigen::Map<Eigen::MatrixXd> pi(v.data(), demography.size(), p_susceptibility.cols());
  return pi_tmp.array() * psusc.array();
}
